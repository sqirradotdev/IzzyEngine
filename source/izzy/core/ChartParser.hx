...
import izzy.core.Conductor.EventStruct;
import izzy.core.Conductor.TimeEvents;
import izzy.state.PlayState.GameplayEvents;
import sys.io.File;
	...
	public static function readVanilla(contents:String):ChartData
	{
		...
		for (notes in sectionInformation)
		{
			...
			for (sectionNote in sectionNotes)
			{
				...
				var tick:Int = Math.round(Conductor.getTick(noteTime));
